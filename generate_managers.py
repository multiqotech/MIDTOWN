...
BASE_DIR = "/home/krishna-gupta/Desktop/MIDTOWN/admin/src/app/dashboard"

singleton_template = """'use client';
import React, {{ useState, useEffect, useRef }} from 'react';
import {{ CheckCircle2 }} from 'lucide-react';
import {{ api }} from '../../../lib/api';

export default function {component_name}() {{
  const [data, setData] = useState<any>({{ heroTitle: '', heroSubtitle: '', heroImage: '' }});
          ...
          <form onSubmit={{handleSave}} style={{{{ display: 'grid', gap: '1.5rem' }}}}>
            ...
            <div style={{{{ display: 'flex', gap: '1rem' }}}}>
              <div style={{{{ flex: 1 }}}}>
                ...
                <input type="text" value={{editingItem?.issuerOrRole || ''}} onChange={{(e) => setEditingItem((prev: any) => ({{ ...prev!, issuerOrRole: e.target.value }}))}} style={{{{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}}} />
              </div>
              <div style={{{{ flex: 1 }}}}>
                <label style={{{{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}}}>Year</label>
                <input type="text" value={{editingItem?.year || ''}} onChange={{(e) => setEditingItem((prev: any) => ({{ ...prev!, year: e.target.value }}))}} style={{{{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}}} />
              </div>
            </div>
            <div>
              <label style={{{{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}}}>Description / Content *</label>
              <textarea required value={{editingItem?.description || editingItem?.quote || ''}} onChange={{(e) => setEditingItem((prev: any) => ({{ ...prev!, description: e.target.value, quote: e.target.value }}))}} style={{{{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none', minHeight: '120px' }}}} />
            </div>
            <div>
              <label style={{{{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}}}>Image URL</label>
              <input type="text" value={{editingItem?.image || editingItem?.imageUrl || ''}} onChange={{(e) => setEditingItem((prev: any) => ({{ ...prev!, image: e.target.value, imageUrl: e.target.value }}))}} style={{{{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}}} />
            </div>
...
for endpoint, comp_name, folder, title in SINGLETONS:
    d = os.path.join(BASE_DIR, folder)
    os.makedirs(d, exist_ok=True)
    with open(os.path.join(d, f"{comp_name}.tsx"), "w") as f:
        f.write(singleton_template.format(component_name=comp_name, endpoint=endpoint, title=title))

for endpoint, comp_name, folder, title, item_name in CRUDS:
    d = os.path.join(BASE_DIR, folder)
    os.makedirs(d, exist_ok=True)
    with open(os.path.join(d, f"{comp_name}.tsx"), "w") as f:
        f.write(crud_template.format(component_name=comp_name, endpoint=endpoint, title=title, item_name=item_name))
