...
export default function PressReleasesManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/press-releases');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e: any) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/press-releases/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/press-releases', editingItem);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/api/discover/press-releases/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage PressReleases</h3>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit' : 'Add'} Press Release
            </h3>
