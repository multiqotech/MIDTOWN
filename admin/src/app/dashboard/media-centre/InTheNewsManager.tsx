...
export default function InTheNewsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/in-the-news');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e: any) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/in-the-news/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/in-the-news', editingItem);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/api/discover/in-the-news/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage InTheNews</h3>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit' : 'Add'} News Article
            </h3>
