...
export default function EmployeeSpotlightsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/employee-spotlights');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/employee-spotlights/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/employee-spotlights', editingItem);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/discover/employee-spotlights/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Employee Spotlights</h3>
