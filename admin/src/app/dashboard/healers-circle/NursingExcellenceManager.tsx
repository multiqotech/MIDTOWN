...
export default function NursingExcellenceManager() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/api/discover/nursing-excellence');
      if (res) setData(res);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      await api.put('/api/discover/nursing-excellence', data);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
        <h3 style={{ margin: '0 0 2rem', color: '#fff' }}>Nursing Excellence</h3>
