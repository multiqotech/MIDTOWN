...
export default function ClinicalIndicatorsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/clinical-indicators');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/clinical-indicators/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/clinical-indicators', editingItem);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/discover/clinical-indicators/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Clinical Indicators</h3>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit' : 'Add'} indicator
            </h3>
