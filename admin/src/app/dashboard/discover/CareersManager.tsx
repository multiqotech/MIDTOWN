...
export default function CareersManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/jobs');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const openNewForm = () => {
    setEditingItem({ title: '', slug: '', department: '', location: '', employmentType: '', experienceRequired: '', shortDescription: '', fullDescription: '', requirements: [], responsibilities: [], applicationLink: '', status: '' });
    setIsFormOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/jobs/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/jobs', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/jobs/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Jobs / Careers</h3>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff' }}>{editingItem?._id ? 'Edit' : 'Add'} Jobs / Careers</h3>
