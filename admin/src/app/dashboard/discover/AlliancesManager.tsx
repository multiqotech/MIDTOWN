...
export default function AlliancesManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/alliances');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const openNewForm = () => {
    setEditingItem({ partnerName: '', slug: '', partnerLogo: '', category: '', description: '', websiteUrl: '', publishedStatus: '' });
    setIsFormOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/alliances/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/alliances', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/alliances/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Alliances</h3>
            ...
            {items.map((item) => (
              <div key={item._id} style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a', overflow: 'hidden', padding: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    <h4 style={{ margin: '0 0 0.5rem', color: '#fff' }}>{item.partnerName}</h4>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff' }}>{editingItem?._id ? 'Edit' : 'Add'} Alliances</h3>
          ...
          <form onSubmit={handleSave} style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <div>
              ...
              <textarea 
                ...
                onChange={(e: any) => setEditingItem((prev: any) => ({ ...prev!, partnerName: e.target.value }))}
            ...
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Website URL *</label>
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
                <input 
                  type="text" 
                  value={editingItem?.websiteUrl || ''}
                  onChange={(e: any) => setEditingItem((prev: any) => ({ ...prev!, websiteUrl: e.target.value }))}
                  style={{ flex: 1, padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}
                  placeholder="Enter URL or upload file"
                />
                <button 
                  type="button"
                  onClick={() => { setCurrentUploadField('websiteUrl'); fileInputRef.current?.click(); }}
                  style={{ backgroundColor: '#222', color: '#fff', border: '1px solid #333', padding: '0 1.5rem', borderRadius: '8px', cursor: 'pointer' }}
                >
                  Upload
                </button>
              </div>
              {editingItem?.websiteUrl && (
                <div style={{ width: '150px', height: '150px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #333' }}>
                  <img src={editingItem.websiteUrl} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              )}
            </div>
