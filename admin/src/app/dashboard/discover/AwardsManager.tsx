...
export default function AwardsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/awards');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const openNewForm = () => {
    setEditingItem({ awardName: '', awardingOrganization: '', year: '', category: '', description: '', certificateImage: '', publishedStatus: '' });
    setIsFormOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/awards/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/awards', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/awards/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Awards</h3>
            ...
            {items.map((item) => (
              <div key={item._id} style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a', overflow: 'hidden', padding: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    <h4 style={{ margin: '0 0 0.5rem', color: '#fff' }}>{item.awardName}</h4>
                    <p style={{ margin: 0, color: '#aaa', fontSize: '0.9rem' }}>{item.awardingOrganization}</p>
                  </div>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff' }}>{editingItem?._id ? 'Edit' : 'Add'} Awards</h3>
          ...
          <form onSubmit={handleSave} style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Certificate Image URL *</label>
