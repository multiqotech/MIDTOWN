...
export default function GroupBrandsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/group-brands');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const openNewForm = () => {
    setEditingItem({ brandName: '', slug: '', logo: '', description: '', category: '', websiteUrl: '', publishedStatus: '' });
    setIsFormOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/group-brands/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/group-brands', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/group-brands/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Group Brands</h3>
            ...
            {items.map((item) => (
              <div key={item._id} style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a', overflow: 'hidden', padding: '1.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    <h4 style={{ margin: '0 0 0.5rem', color: '#fff' }}>{item.brandName}</h4>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff' }}>{editingItem?._id ? 'Edit' : 'Add'} Group Brands</h3>
          ...
          <form onSubmit={handleSave} style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Brand Name *</label>
              <textarea 
                value={editingItem?.brandName || ''}
                onChange={(e: any) => setEditingItem((prev: any) => ({ ...prev!, brandName: e.target.value }))}
            ...
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Logo URL *</label>
              <textarea 
                value={editingItem?.logo || ''}
                onChange={(e: any) => setEditingItem((prev: any) => ({ ...prev!, logo: e.target.value }))}
                style={{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}
              />
            </div>
