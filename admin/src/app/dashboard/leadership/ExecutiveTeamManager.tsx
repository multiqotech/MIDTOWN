...
export default function ExecutiveTeamManager() {
  const [members, setMembers] = useState<ExecutiveTeamMember[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingMember, setEditingMember] = useState<ExecutiveTeamMember | null>(null);
  ...
  const openEditForm = (member: ExecutiveTeamMember) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingMember._id) {
        await api.put(`/api/discover/executive-team/${editingMember._id}`, editingMember);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/executive-team', editingMember);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/executive-team/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Executive Team</h3>
            <button 
              onClick={openNewForm}
              style={{
                display: 'flex',
            ...
            {members.map((member) => (
              <div key={member._id} style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '1.5rem' }}>
                  ...
                  <p style={{ margin: '0 0 0.5rem', color: '#00A676', fontSize: '0.9rem', fontWeight: '500' }}>{member.designation} - {member.department}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={(e: any) => setEditingMember(prev => ({ ...prev!, department: e.target.value }))}
                  style={{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}
                />
              </div>
              <div>
