...
export default function CorporateTieUpsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/corporate-tie-ups');
      setItems(data);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e: any) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/corporate-tie-ups/${editingItem._id}`, editingItem);
        showMessage('Updated successfully!', 'success');
      } else {
        await api.post('/api/discover/corporate-tie-ups', editingItem);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/api/discover/corporate-tie-ups/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage CorporateTieUps</h3>
            <button onClick={openNewForm} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: '#00A676', color: 'white', border: 'none', padding: '0.75rem 1.25rem', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>
              <Plus size={18} /> Add Corporate Tie-up
            </button>
          </div>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit' : 'Add'} Corporate Tie-up
            </h3>
