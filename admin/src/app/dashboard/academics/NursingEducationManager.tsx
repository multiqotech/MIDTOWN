...
export default function NursingEducationManager() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/api/discover/nursing-education');
      if (res) setData(res);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.put('/api/discover/nursing-education', data);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
        <h3 style={{ margin: '0 0 2rem', color: '#fff' }}>Nursing Education</h3>
