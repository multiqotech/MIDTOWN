...
export default function CorporateAnnouncementsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/corporate-announcements');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/corporate-announcements/${editingItem._id}`, editingItem);
        showMessage('Announcement updated successfully!', 'success');
      } else {
        await api.post('/api/discover/corporate-announcements', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/corporate-announcements/${id}`);
  ...
  if (loading) return <div style={{ color: '#00A676' }}>Loading Announcements...</div>;
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Announcements</h3>
            <button 
              onClick={openNewForm}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {items.map((item) => (
              <div key={item._id} style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a', padding: '1.5rem', position: 'relative' }}>
                ...
                <p style={{ margin: '0 0 0.5rem 0', color: '#aaa', fontSize: '0.95rem' }}><strong>Date:</strong> {item.date}</p>
                ...
                <p style={{ margin: '0 0 0.5rem 0', color: '#aaa', fontSize: '0.95rem' }}><strong>Link (Optional URL):</strong> {item.link}</p>
              </div>
            ))}
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit' : 'Add New'} Announcement
            </h3>
          ...
          <form onSubmit={handleSave} style={{ display: 'grid', gap: '1.5rem' }}>
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Date *</label>
              <input 
                required
                type="text" 
                value={editingItem?.date || ''}
                onChange={(e: any) => setEditingItem(prev => ({ ...prev!, date: e.target.value }))}
            ...
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>Link (Optional URL) *</label>
              <input 
                required
                type="text" 
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
              ...
              <button type="submit" style={{ backgroundColor: '#00A676', color: '#fff', border: 'none', padding: '0.75rem 2.5rem', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>Save Announcement</button>
            </div>
          </form>
        </div>
      )}
