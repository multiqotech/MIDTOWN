...
export default function AnnualReportsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/annual-reports');
      setItems(Array.isArray(data) ? data : (data.data || []));
    } catch (err) {
      console.error(err);
      showMessage('Failed to load Annual Reports', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/annual-reports/${editingItem._id}`, editingItem);
        showMessage('Annual Report updated successfully!', 'success');
      } else {
        await api.post('/api/discover/annual-reports', editingItem);
        showMessage('Annual Report added successfully!', 'success');
      }
      closeForm();
      fetchItems();
    } catch (err) {
      console.error(err);
      showMessage('Failed to save Annual Report', 'error');
    }
  };
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/annual-reports/${id}`);
      showMessage('Annual Report deleted successfully!', 'success');
      fetchItems();
    } catch (err) {
      console.error(err);
      showMessage('Failed to delete Annual Report', 'error');
    }
  };

  if (loading) return <div style={{ color: '#00A676' }}>Loading Annual Reports...</div>;
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Manage Annual Reports</h3>
            <button 
              onClick={openNewForm}
            ...
            >
              <Plus size={18} /> Add Annual Report
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {items.map((item) => (
              <div key={item._id} style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a', padding: '1.5rem', position: 'relative' }}>
                ...
                <p style={{ margin: '0 0 0.5rem 0', color: '#aaa', fontSize: '0.95rem' }}><strong>Title:</strong> {item.title}</p>
                <p style={{ margin: '0 0 0.5rem 0', color: '#aaa', fontSize: '0.95rem' }}><strong>PDF Link (URL):</strong> {item.pdfLink}</p>
              </div>
            ))}
            
            {items.length === 0 && (
              <div style={{ padding: '3rem', textAlign: 'center', color: '#888', backgroundColor: '#1A1A1A', borderRadius: '12px', border: '1px solid #2a2a2a' }}>
                No Annual Reports found. Create one to get started!
              </div>
            )}
          </div>
        </>
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit' : 'Add New'} Annual Report
            </h3>
          ...
          <form onSubmit={handleSave} style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <div>
              <label style={{ display: 'block', color: '#aaa', fontSize: '0.9rem', marginBottom: '0.5rem' }}>PDF Link (URL) *</label>
              <input 
                required
                type="text" 
                value={editingItem?.pdfLink || ''}
                onChange={(e: any) => setEditingItem(prev => ({ ...prev!, pdfLink: e.target.value }))}
                style={{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none' }}
              />
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
              ...
              <button type="submit" style={{ backgroundColor: '#00A676', color: '#fff', border: 'none', padding: '0.75rem 2.5rem', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>Save Annual Report</button>
            </div>
          </form>
        </div>
      )}
