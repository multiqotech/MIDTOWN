...
export default function EthicsComplianceManager() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/api/discover/ethics-compliance');
      if (res) setData(res);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.put('/api/discover/ethics-compliance', data);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
        <h3 style={{ margin: '0 0 2rem', color: '#fff' }}>EthicsCompliance</h3>
        ...
        <form onSubmit={handleSave} style={{ display: 'grid', gap: '1.5rem' }}>
            <div>
              ...
              <textarea 
                value={data.title || ''}
                onChange={(e: any) => setData((prev: any) => ({ ...prev!, title: e.target.value }))}
            ...
            <div>
              ...
              <textarea 
                value={data.content || ''}
                onChange={(e: any) => setData((prev: any) => ({ ...prev!, content: e.target.value }))}
                style={{ width: '100%', padding: '0.875rem', backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', color: '#fff', outline: 'none', minHeight: '80px' }}
              />
            </div>
