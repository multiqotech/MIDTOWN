...
export default function GreenHospitalsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/green-hospitals');
      setItems(data);
    } catch (err) {
      console.error(err);
      showMessage('Failed to load Green Hospitals items', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/green-hospitals/${editingItem._id}`, editingItem);
        showMessage('Item updated successfully!', 'success');
      } else {
        await api.post('/api/discover/green-hospitals', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/green-hospitals/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Green Hospitals Initiatives</h3>
            <button 
              onClick={openNewForm}
              style={{
                display: 'flex',
