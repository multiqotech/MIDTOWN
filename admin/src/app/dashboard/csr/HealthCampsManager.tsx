...
export default function HealthCampsManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/health-camps');
      setItems(data);
    } catch (err) {
      console.error(err);
      showMessage('Failed to load Health Camps', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/health-camps/${editingItem._id}`, editingItem);
        showMessage('Item updated successfully!', 'success');
      } else {
        await api.post('/api/discover/health-camps', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/health-camps/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Health Camps</h3>
            <button 
              onClick={openNewForm}
              style={{
                display: 'flex',
        ...
        <div style={{ backgroundColor: '#1A1A1A', borderRadius: '12px', padding: '2.5rem', border: '1px solid #2a2a2a' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>
              {editingItem?._id ? 'Edit Health Camp' : 'Add New Health Camp'}
            </h3>
            <button 
              onClick={closeForm}
