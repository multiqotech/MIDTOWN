...
export default function CommunityOutreachManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/community-outreach');
      setItems(data);
    } catch (err) {
      console.error(err);
      showMessage('Failed to load Community Outreach initiatives', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/community-outreach/${editingItem._id}`, editingItem);
        showMessage('Item updated successfully!', 'success');
      } else {
        await api.post('/api/discover/community-outreach', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/community-outreach/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Community Outreach</h3>
            <button 
              onClick={openNewForm}
              style={{
                display: 'flex',
