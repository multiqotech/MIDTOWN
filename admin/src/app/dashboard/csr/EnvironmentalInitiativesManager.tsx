...
export default function EnvironmentalInitiativesManager() {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.get('/api/discover/environmental-initiatives');
      setItems(data);
    } catch (err) {
      console.error(err);
      showMessage('Failed to load Environmental Initiatives', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem._id) {
        await api.put(`/api/discover/environmental-initiatives/${editingItem._id}`, editingItem);
        showMessage('Item updated successfully!', 'success');
      } else {
        await api.post('/api/discover/environmental-initiatives', editingItem);
  ...
  const handleDelete = async (id: string | undefined) => {
    ...
    try {
      await api.delete(`/api/discover/environmental-initiatives/${id}`);
      ...
      {!isFormOpen ? (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff', fontSize: '1.25rem' }}>Environmental Initiatives</h3>
            <button 
              onClick={openNewForm}
              style={{
                display: 'flex',
