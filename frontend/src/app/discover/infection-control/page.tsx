...
export default function InfectionControlPage() {
  ...
  useEffect(() => {
    api.get('/api/discover/infection-control')
    ...
    <>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          {data?.heroImage && <Image src={data.heroImage} alt={data.heroTitle || 'Infection Control'} fill style={{ objectFit: 'cover' }} priority />}
          <div className={styles.overlay} />
          <div className={styles.heroContent}>
            <h1 className={styles.title}>{data?.heroTitle || 'Infection Control'}</h1>
            <p className={styles.subtitle}>{data?.heroSubtitle || 'Our rigorous protocols for maintaining the highest standards of hygiene and safety.'}</p>
          </div>
        </section>
