...
export default function FeedbackMechanismPage() {
  const [data, setData] = useState<PageData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/api/discover/feedback-mechanism')
    ...
    <>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          {data?.heroImage && <Image src={data.heroImage} alt={data.heroTitle || 'Feedback Mechanism'} fill style={{ objectFit: 'cover' }} priority />}
          <div className={styles.overlay} />
          <div className={styles.heroContent}>
            <h1 className={styles.title}>{data?.heroTitle || 'Feedback Mechanism'}</h1>
            <p className={styles.subtitle}>{data?.heroSubtitle || 'Your voice matters — shaping better healthcare through continuous feedback.'}</p>
          </div>
        </section>
