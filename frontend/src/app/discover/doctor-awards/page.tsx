...
export default function DoctorAwardsPage() {
  ...
  useEffect(() => {
    api.get('/api/discover/doctor-awards')
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          ...
          <div className={styles.heroContent}>
            <h1 className={styles.title}>Doctor Awards</h1>
            <p className={styles.subtitle}>Honouring our physicians for outstanding contributions to patient care.</p>
          </div>
        </section>
              ...
              {items.map((item: any) => (
                <div key={item._id} className={styles.card}>
                  ...
                  <div className={styles.cardBody}>
                    ...
                    {item.slug && (
                      <Link href={`/discover/doctor-awards/${item.slug}`} className={styles.cardLink}>
                        View Details →
                      </Link>
                    )}
                  </div>
