...
export default function EmployeeSpotlightsPage() {
  ...
  useEffect(() => {
    api.get('/api/discover/employee-spotlights')
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          ...
          <div className={styles.heroContent}>
            <h1 className={styles.title}>Employee Spotlights</h1>
            <p className={styles.subtitle}>Stories of dedication and excellence from the Midtown family.</p>
          </div>
        </section>
              ...
              {items.map((item: any) => (
                <div key={item._id} className={styles.card}>
                  ...
                  <div className={styles.cardBody}>
                    ...
                    {item.slug && (
                      <Link href={`/discover/employee-spotlights/${item.slug}`} className={styles.cardLink}>
                        View Details →
                      </Link>
                    )}
                  </div>
