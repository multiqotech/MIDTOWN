...
export default function NursingExcellencePage() {
  const [data, setData] = useState<PageData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/api/discover/nursing-excellence')
    ...
    <>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          {data?.heroImage && <Image src={data.heroImage} alt={data.heroTitle || 'Nursing Excellence'} fill style={{ objectFit: 'cover' }} priority />}
          <div className={styles.overlay} />
          <div className={styles.heroContent}>
            <h1 className={styles.title}>{data?.heroTitle || 'Nursing Excellence'}</h1>
            <p className={styles.subtitle}>{data?.heroSubtitle || 'Celebrating the compassion and clinical mastery of our nursing professionals.'}</p>
          </div>
        </section>
