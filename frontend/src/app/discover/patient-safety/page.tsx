...
export default function PatientSafetyPage() {
  ...
  useEffect(() => {
    api.get('/api/discover/patient-safety')
    ...
    <>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          {data?.heroImage && <Image src={data.heroImage} alt={data.heroTitle || 'Patient Safety'} fill style={{ objectFit: 'cover' }} priority />}
          <div className={styles.overlay} />
          <div className={styles.heroContent}>
            <h1 className={styles.title}>{data?.heroTitle || 'Patient Safety'}</h1>
            <p className={styles.subtitle}>{data?.heroSubtitle || 'Comprehensive patient safety programs that put wellbeing first.'}</p>
          </div>
        </section>
