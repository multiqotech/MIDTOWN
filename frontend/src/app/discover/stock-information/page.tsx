...
export default function StockInformationPage() {
  ...
  useEffect(() => {
    api.get('/api/discover/stock-information')
    ...
    <>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          {data?.heroImage && <Image src={data.heroImage} alt={data.heroTitle || 'Stock Information'} fill style={{ objectFit: 'cover' }} priority />}
          <div className={styles.overlay} />
          <div className={styles.heroContent}>
            <h1 className={styles.title}>{data?.heroTitle || 'Stock Information'}</h1>
            <p className={styles.subtitle}>{data?.heroSubtitle || 'Real-time market data and investment insights.'}</p>
          </div>
        </section>
