...
export default function ClinicalIndicatorsPage() {
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/api/discover/clinical-indicators')
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          ...
          <div className={styles.heroContent}>
            <h1 className={styles.title}>Clinical Indicators</h1>
            <p className={styles.subtitle}>Measurable outcomes demonstrating our clinical quality and effectiveness.</p>
          </div>
        </section>
              ...
              {items.map((item: any) => (
                <div key={item._id} className={styles.card}>
                  ...
                  <div className={styles.cardBody}>
                    ...
                    {item.slug && (
                      <Link href={`/discover/clinical-indicators/${item.slug}`} className={styles.cardLink}>
                        View Details →
                      </Link>
                    )}
                  </div>
