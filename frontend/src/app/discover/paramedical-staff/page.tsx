...
export default function ParamedicalStaffPage() {
  ...
  useEffect(() => {
    api.get('/api/discover/paramedical-staff')
    ...
    <>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          {data?.heroImage && <Image src={data.heroImage} alt={data.heroTitle || 'Paramedical Staff'} fill style={{ objectFit: 'cover' }} priority />}
          <div className={styles.overlay} />
          <div className={styles.heroContent}>
            <h1 className={styles.title}>{data?.heroTitle || 'Paramedical Staff'}</h1>
            <p className={styles.subtitle}>{data?.heroSubtitle || 'Recognising the vital role of our paramedical healthcare heroes.'}</p>
          </div>
        </section>
